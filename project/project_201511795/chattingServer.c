...
main(int argc, char *argv[])
{
	...
	char clientName[MAX_CLIENT][20];	// clients name변수
	...
	if(argc != 2)
	{
		err_quit("usage:  <Port>"); // 파라미터 오류
	}



	// <-- listen socket
	l_socket = Socket(AF_INET, SOCK_STREAM, 0);     // 리스닝 소켓

	bzero(&l_addr, sizeof(l_addr) );                // 초기화

	l_addr.sin_addr.s_addr = htonl(INADDR_ANY);     // 주소 초기화
	l_addr.sin_family = AF_INET;                    // 주소 초기화
	l_addr.sin_port = htons(atoi(argv[1]));         // 주소 초기화

	Bind(l_socket, (SA *) &l_addr, sizeof(l_addr)); // 바인딩

	Listen(l_socket, LISTENQ);                      // 리스닝
	// listen socket--> 
		...
		for(i = 0; i < MAX_CLIENT; i++)
		{
			if( (client[i] != -1) && FD_ISSET(client[i], &rset) )
			{
				...
				if( (n = read(client[i], chatData, sizeof(chatData))) > 0)
				{
					...
					if(strstr(chatData,list) != NULL)
					{
						...
						{   
							...
							for(k = 0; k < MAX_CLIENT; k++)
							{
								...
								{
                                    char e[30];
									snprintf(e,sizeof(e),"[smsg from %s] ", token);
                                    ...
                                    snprintf(buf,sizeof(buf),"your insert to port number >> %s\n argument error, /smsg [port] [send to message]\n", token);
									write(client[k], buf, strlen(buf));
                                }
								break;
							}
                    ...
                    int len = sizeof(cliaddr);    // all user socketfd length
                    getpeername(client[i], (SA *) &cliaddr, &len);    // get user info
